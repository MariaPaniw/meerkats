...
### R script to project meerkat IPMs to future rainfall and temperature deviation scenarios 
...
# Intial female population vector (Here Dec 2016)
load("/Users/maria/Dropbox/Meerkats/SuppMat/N0_2016.rda")

dens0=sum(N0)/57.9 # initial density - total number of females/population range (Dec 2016)

# Future observed (1997-2016) temperature and rainfall deviation

load("/Users/maria/Dropbox/Meerkats/SuppMat/clim.obs.rda")

### Start simulations

n.simul=3 # in the main manuscript, we performed 100 simulations, but doing so will signficantly slow simulations 
n.years=50
av_month=1:12
...
### # load year simulations from base
## Here, we performed exactly the same simulations as below, but for each simulation year, 
## we randomly sampled one of the modeled year (1997-2016),
## went sequentially through all the months and modeled observed (1997-2016) temperature and rainfall deviations
## running the projection scenario "base" below, demonstrates how this is done 

load("/Users/maria/Dropbox/Meerkats/SuppMat/year.sim")

## Load all necessary climatic variables (which can be also be obtained by running climData.R)

load("/Users/maria/Dropbox/Meerkats/SuppMat/stoch.month.Rain")

load("/Users/maria/Dropbox/Meerkats/SuppMat/stoch.month.Temp")

# Empty arrays to hold simulation results in 
mass.dist=array(0,c(length(N0),n.years,length(av_month),n.simul)) # distribution of x-mass individuals per stage
PR=array(0,c(n.years,length(av_month),n.simul)) # simulated population range
densSim=array(0,c(n.years,length(av_month),n.simul)) # simulated density
dominants=array(0,c(n.years,length(av_month),n.simul)) # simulated number of dominant females
extinction=array(0,c(n.years,length(av_month),n.simul))

# Begin building IPMs 
## choose your projection scenario (Supplementary material S4):

# base - see above
# seq - sequentially pick projected rain/temp deviations - picked here
# stoch - stochastic picks of rain/temp deviations 
# dry - simulate consitently drier conditions (pick the lowest relative rainfall deviation projected for a focal month)

proj.scen= c("base","seq","stoch","dry")[2]

# To descrease simulation time, we are also modeling only RCP2.6 here 
for(si in 1:n.simul){
  N1=N0
  dens1=dens0
  
  for(xx in 1:n.years){ # loop over 50 simulation years (2017-2066)
    
    if(proj.scen%in%c("seq","stoch", "rain")){
      
      j=which(av_year%in%year.sim[xx,si]) # pick year from base simulations
    }else{
      
      j=sample(1:length(av_year),1) # sample a year for base simulations
    }
    ...
    for(i in 1:length(av_month)){ # loop over months - 1-12 in sequence
      ...
      if(proj.scen=="stoch"){
        ...
        tempSD=clim.obs$tempSD[clim.obs$year==av_year[j]&clim.obs$month==av_month[i]] + stoch.month.Temp[j,av_month[i],pick,1]# temperature standardized deviation for each month and year
        rainSD=clim.obs$rainSD[clim.obs$year==av_year[j]&clim.obs$month==av_month[i]]+ stoch.month.Rain[j,av_month[i],pick,1] # rainfall standardized deviation for each month and year
     
      }else if(proj.scen=="seq"){
        
        tempSD=clim.obs$tempSD[clim.obs$year==av_year[j]&clim.obs$month==av_month[i]] + stoch.month.Temp[j,av_month[i],si,1]# temperature standardized deviation for each month and year
        rainSD=clim.obs$rainSD[clim.obs$year==av_year[j]&clim.obs$month==av_month[i]]+ stoch.month.Rain[j,av_month[i],si,1] # rainfall standardized deviation for each month and year
        
      }else if(proj.scen=="dry"){
        
        sampleSize=stoch.month.Rain[j,av_month[i],,1][stoch.month.Rain[j,av_month[i],,1]<0] # check whether there are negative relative deviations for rain for a given month
        
        # if yes, sample one the negative relative deviations, if not, take the minimum relative deviation 
        if(length(sampleSize)>0) newRain<-sample(sampleSize,1) else newRain <- min(stoch.month.Rain[j,av_month[i],,1])
        pick=which(stoch.month.Rain[j,av_month[i],,1]==newRain)
        ...
        tempSD=clim.obs$tempSD[clim.obs$year==av_year[j]&clim.obs$month==av_month[i]] 
        rainSD=clim.obs$rainSD[clim.obs$year==av_year[j]&clim.obs$month==av_month[i]]
      }
      ...
      mass.dist[,xx,i,si]= N1
      PR[xx,i,si]=poprange
      densSim[xx,i,si]=dens1 # multiply by two to add males 
      dominants[xx,i,si]=doms
      if(sum(N1)<20|doms<5) extinction[xx,i,si] <-1
    }
...
dyn.sim=list(mass.dist=mass.dist,PR=PR,densSim=densSim,dominants=dominants) 
