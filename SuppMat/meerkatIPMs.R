...
file.names <- list.files(path = "/Users/mariapaniw/Dropbox/Meerkats/GAMs")

for(i in 1:length(file.names)){
  
  load(paste("/Users/mariapaniw/Dropbox/Meerkats/GAMs/",file.names[i],sep=""))
}
...
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/N0.rda")
...
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/clim.obs.rda")
...
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/dyn.obs")
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/dens.obs.rda")
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/prop.stage.obs.rda")
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/pop.range.obs.rda")
...
dens.sim$date=dens.sim$date%m+% months(1)
...
# Check correlation - removing the seasonal component 
library(fpp)
trend.sim = ma(dens.sim$density, order = 12, centre = T)
detrend.sim=as.ts(dens.sim$density)/trend.sim
seasonal.sim = colMeans(t(matrix(data = detrend.sim, nrow = 12)), na.rm = T)
random.sim=as.ts(dens.sim$density)/(trend.sim*seasonal.sim)

trend.obs = ma(dens.obs$density, order = 12, centre = T)
detrend.obs=as.ts(dens.obs$density)/trend.obs
seasonal.obs = colMeans(t(matrix(data = detrend.obs, nrow = 12)), na.rm = T)
random.obs=as.ts(dens.obs$density)/(trend.obs*seasonal.obs)

cor.test(random.sim,random.obs)# significant correlation 
...
### PLOT PROPORTION OF DIFFERENT LIFE-HISTORY STAGES

load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/dist.stage.sim")
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/prop.stage.obs.rda")

stage.sim=adply(dist.stage.sim,c(1,2,3))
colnames(stage.sim)=c("stage","year","month","prop")
levels(stage.sim$stage)=c("Pups","Juv","SubA","NPH","PH","HL","NPD","PD","DL")
levels(stage.sim$year)=1997:2016
stage.sim$date=as.Date(paste(stage.sim$year,stage.sim$month,"15",sep="-"))
stage.sim=stage.sim[order(stage.sim$date),]
stage.sim$date=stage.sim$date%m+% months(1)
stage.sim$cat="Projected"
stage.sim=droplevels(stage.sim[stage.sim$date>"1997-12-15",c(1,4,5,6)]) # remove first year because projections for stages haven't stabilized 

stage.obs=prop.stage.obs[,c(2,5,1)]

stage.sim$prop.obs=left_join(stage.sim,stage.obs,by=c("date","stage"))$prop.y

stage.obs=stage.sim[,c(1,3,5)]
colnames(stage.obs)[3]="prop"
stage.obs$cat="Observed"
stage.obs$prop[is.na(stage.obs$prop)]=0

all=rbind(stage.sim[,1:4],stage.obs[,c(1,3,2,4)])

ggplot(data=all,aes(date,prop,col=cat,group=cat))+
  geom_line(size=1.5)+
...
stages=c("Pups","Juv","SubA","NPH","PH","HL","NPD","PD","DL")

for(i in 1:length(stages)){
  print(paste("Simple Correlation", stages[i]))
  print(cor.test(stage.sim$prop[stage.sim$stage==stages[i]&stage.sim$date<"2017-01-15"],stage.obs$prop[stage.obs$stage==stages[i]&stage.obs$date<"2017-01-15"]))
  
  # Check correlation - removing the seasonal component 
  trend.sim = ma(stage.sim$prop[stage.sim$stage==stages[i]&stage.sim$date<"2017-01-15"], order = 12, centre = T)
  detrend.sim=as.ts(stage.sim$prop[stage.sim$stage==stages[i]&stage.sim$date<"2017-01-15"])/trend.sim
  seasonal.sim = colMeans(t(matrix(data = detrend.sim, nrow = 12)), na.rm = T)
  random.sim=as.ts(stage.sim$prop[stage.sim$stage==stages[i]&stage.sim$date<"2017-01-15"])/(trend.sim*seasonal.sim)
  
  trend.obs = ma(stage.obs$prop[stage.obs$stage==stages[i]&stage.obs$date<"2017-01-15"], order = 12, centre = T)
  detrend.obs=as.ts(stage.obs$prop[stage.obs$stage==stages[i]&stage.obs$date<"2017-01-15"])/trend.obs
  seasonal.obs = colMeans(t(matrix(data = detrend.obs, nrow = 12)), na.rm = T)
  random.obs=as.ts(stage.obs$prop[stage.obs$stage==stages[i]&stage.obs$date<"2017-01-15"])/(trend.obs*seasonal.obs)
  
  print(paste("Detrended Correlation", stages[i]))
  print(cor.test(random.sim,random.obs))
  
}


### MASS DISTRIBUTION PER STAGE

load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/mean.mass.sim")
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/se.mass.sim")

mean.mass=adply(mean.mass.sim,c(1,2,3))
rm(mean.mass.sim)
colnames(mean.mass)=c("stage","year","month","mass")
levels(mean.mass$stage)=c("Pups","Juv","SubA","NPH","PH","HL","NPD","PD","DL")
mean.mass$se=adply(se.mass.sim,c(1,2,3))[,4]
levels(mean.mass$year)=1997:2016
mean.mass$date=as.Date(paste(mean.mass$year,mean.mass$month,"15",sep="-"))
mean.mass=mean.mass[order(mean.mass$date),]
mean.mass$date=mean.mass$date%m+% months(1)
mean.mass$cat="Projected"

mean.mass=droplevels(mean.mass[mean.mass$date>"1997-12-15",]) # remove first year because projections for stages haven't stabilized 

load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/mean.mass.obs.rda")

mean.mass$mass.obs=left_join(mean.mass,mean.mass.obs,by=c("date","stage"))$mass.y
mean.mass$se.obs=left_join(mean.mass,mean.mass.obs,by=c("date","stage"))$se.y

mass.obs=mean.mass[,c(1,6,8,9)]
colnames(mass.obs)[3:4]=c("mass","se")
mass.obs$cat="Observed"
all=rbind(mean.mass[,c(1,4,5,6,7)],mass.obs)

all$stage=factor(all$stage,levels=c("Pups","Juv","SubA","NPH","PH","HL","NPD","PD","DL"))

ggplot(data=all,aes(date,mass,col=cat,group=cat))+
  geom_point(size=1.5)+
...
for(i in 1:length(stages)){
  print(paste("Simple Correlation", stages[i]))
  print(cor.test(mean.mass$mass[mean.mass$stage==stages[i]&mean.mass$date<"2017-01-15"],mass.obs$mass[mass.obs$stage==stages[i]&mass.obs$date<"2017-01-15"]))
  
  if(i!=6){ # too few data for HL
    # Check correlation - removing the seasonal component 
    trend.sim = ma(mean.mass$mass[mean.mass$stage==stages[i]&mean.mass$date<"2017-01-15"], order = 12, centre = T)
    detrend.sim=as.ts(mean.mass$mass[mean.mass$stage==stages[i]&mean.mass$date<"2017-01-15"])/trend.sim
    seasonal.sim = colMeans(t(matrix(data = detrend.sim, nrow = 12)), na.rm = T)
    random.sim=as.ts(mean.mass$mass[mean.mass$stage==stages[i]&mean.mass$date<"2017-01-15"])/(trend.sim*seasonal.sim)
    
    trend.obs = ma(mass.obs$mass[mass.obs$stage==stages[i]&mass.obs$date<"2017-01-15"], order = 12, centre = T)
    detrend.obs=as.ts(mass.obs$mass[mass.obs$stage==stages[i]&mass.obs$date<"2017-01-15"])/trend.obs
    seasonal.obs = colMeans(t(matrix(data = detrend.obs, nrow = 12)), na.rm = T)
    random.obs=as.ts(mass.obs$mass[mass.obs$stage==stages[i]&mass.obs$date<"2017-01-15"])/(trend.obs*seasonal.obs)
