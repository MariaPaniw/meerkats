...
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/dom.obs.rda")
load("/Users/mariapaniw/Dropbox/Meerkats/SuppMat/help.obs.rda")
...
# Check correlation - removing the seasonal component 
trend.sim = ma(dom.sim$dom, order = 12, centre = T)
detrend.sim=as.ts(dom.sim$dom)/trend.sim
seasonal.sim = colMeans(t(matrix(data = detrend.sim, nrow = 12)), na.rm = T)
random.sim=as.ts(dom.sim$dom)/(trend.sim*seasonal.sim)

trend.obs = ma(dom.obs$dom, order = 12, centre = T)
detrend.obs=as.ts(dom.obs$dom)/trend.obs
seasonal.obs = colMeans(t(matrix(data = detrend.obs, nrow = 12)), na.rm = T)
random.obs=as.ts(dom.obs$dom)/(trend.obs*seasonal.obs)
cor.test(random.sim,random.obs)# significant correlation 
...
### PLOT PROPORTION OF HELPERS
help.sim=array(0,c(20,12))

for(i in 1:20){
  for(j in 1:12){
    help.sim[i,j]=sum(dyn.obs$mass.dist.obs[1201:1600,i,j])
  }
}
tot.females=apply(dyn.obs$mass.dist.obs,c(2,3),sum)
help.sim=help.sim/tot.females


help.sim=adply(help.sim,c(1,2))
colnames(help.sim)=c("year","month","help")
levels(help.sim$year)=1997:2016
help.sim$date=as.Date(paste(help.sim$year,help.sim$month,"15",sep="-"))
help.sim=help.sim[order(help.sim$date),]
help.sim$cat="Projected"
help.obs$cat="Observed"

cor.test(help.sim$help,help.obs$help)# significant correlation 

# Check correlation - removing the seasonal component 
trend.sim = ma(help.sim$help[-(1:12)], order = 12, centre = T)
detrend.sim=as.ts(help.sim$help[-(1:12)])/trend.sim
seasonal.sim = colMeans(t(matrix(data = detrend.sim, nrow = 12)), na.rm = T)
random.sim=as.ts(help.sim$help[-(1:12)])/(trend.sim*seasonal.sim)

trend.obs = ma(help.obs$help[-(1:12)], order = 12, centre = T)
detrend.obs=as.ts(help.obs$help[-(1:12)])/trend.obs
seasonal.obs = colMeans(t(matrix(data = detrend.obs, nrow = 12)), na.rm = T)
random.obs=as.ts(help.obs$help[-(1:12)])/(trend.obs*seasonal.obs)
cor.test(random.sim,random.obs)# significant correlation (note however that I removed 1997 because the simulations started with a larger number of helpers than observed and it took until 2018 to stabilize)

all=rbind(help.obs,help.sim[,c(4,3,5)])

ggplot(data=all,aes(date,help,col=cat,group=cat))+
  geom_line(size=1.5)+
  scale_color_manual(name="",values=c("black","darkgreen"))+
  theme_bw()+
  theme(legend.title = element_text(size=12, face="bold"),
...
all=rbind(mean.mass.obs,mean.mass[,c(6,1,4,5,7)])
all=all[all$date>"1997-12-15",]
